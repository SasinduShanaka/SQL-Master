...
router.post('/lessons/:id', wrap(async (req, res) => {
  ...
  const user = await users.findByIdAndUpdate(req.user._id, { $addToSet: { [field]: req.params.id } }, { new: true })
...
router.put('/flags/:id', wrap(async (req, res) => {
  ...
  const user = await users.findByIdAndUpdate(req.user._id, { [op]: { flaggedQuestions: req.params.id } }, { new: true })
  res.json(summarizeUser(user))
}))
router.delete('/', wrap(async (req, res) => {
  const user = await users.findByIdAndUpdate(req.user._id, { $set: { startedLessons: [], completedLessons: [], passedQuizzes: [], quizResults: {}, solvedQuestions: [], flaggedQuestions: [], attemptHistory: [] } }, { new: true })
  res.json(summarizeUser(user))
}))
